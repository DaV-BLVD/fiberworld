...
@section('content')
    <div class="container pb-5">
        <h2>Edit Home Internet Plan: {{ $plan->name }}</h2>

        <form action="{{ route('admin.services.home.update', $plan->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
    ...
    <script>
        ...
        @foreach($plan->features as $feature)
            addFeatureRow("{{ $feature->feature_text }}", {{ $feature->is_available ? 'true' : 'false' }});
        @endforeach
    </script>
@endsection